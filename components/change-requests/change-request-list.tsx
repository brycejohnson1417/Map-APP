        ...
        {requests.map((request) => {
          ...
          return (
            ...
            >
              <div className="flex flex-wrap items-center gap-3 px-4 py-4 md:px-5">
                ...
                <div className="ml-auto flex items-center gap-2">
                  <button
                    ...
                    aria-expanded={expanded}
                    aria-controls={`request-details-${request.id}`}
                    aria-label={expanded ? `Collapse ${request.title} details` : `Expand ${request.title} details`}
                  >
              ...
              {expanded ? (
                <div id={`request-details-${request.id}`} className="border-t border-[var(--border-subtle)] bg-[var(--surface-card)] px-4 py-4 md:px-5">
                  {previewImage?.signedUrl ? (
                    <a
                      href={previewImage.signedUrl}
                      target="_blank"
