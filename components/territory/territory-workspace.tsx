...
export function TerritoryWorkspace({ orgSlug, initialDashboard, territoryConfig }: TerritoryWorkspaceProps) {
  ...
  const pinsById = useMemo(() => {
    ...
    // Create a Map dictionary to avoid O(N*M) time complexity from nesting Array.find inside Array.map within routeStops useMemo.
    return new Map(pins.map((pin) => [pin.id, pin]));
  }, [pins]);

  const selectedPin = useMemo(() => pinsById.get(selectedId ?? "") ?? null, [pinsById, selectedId]);
