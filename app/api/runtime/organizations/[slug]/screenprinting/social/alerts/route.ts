...
import { listScreenprintingAlerts } from "@/lib/application/screenprinting/screenprinting-service";
...
export async function GET(_request: Request, context: { params: Promise<{ slug: string }> | { slug: string } }) {
  try {
    return NextResponse.json({ ok: true, ...(await listScreenprintingAlerts(await resolveSlug(context))) });
  } catch (error) {
