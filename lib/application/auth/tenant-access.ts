...
export async function resolveTenantAccess(
  ...
  if (existingMemberships.length) {
    ...
    if (organization) {
      ...
      const workspace = fullOrganization
        ? compileWorkspaceExperience({ slug: fullOrganization.slug, organization: fullOrganization })
        : await getWorkspaceExperienceBySlug(organization.slug);
      return existingWorkspaceAccess({
