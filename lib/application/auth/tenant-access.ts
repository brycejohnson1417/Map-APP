...
export async function resolveTenantAccess(
  ...
  if (existingMemberships.length) {
    ...
    if (organization) {
      ...
      // Performance optimization: We already fetched the organization via organizationsById.
      // Use the synchronous compileWorkspaceExperience instead of getWorkspaceExperienceBySlug
      // to avoid a redundant findBySlug query in the critical auth path.
  ...
  if (emailDomain) {
    ...
    if (workspaceOrganization && (!requestedSlug || workspaceOrganization.slug === requestedSlug)) {
      // Performance optimization: Avoid redundant query for workspaceOrganization
      const compiled = compileWorkspaceExperience({
  ...
  if (guessed) {
    ...
    if (guessed.slug) {
      ...
      if (existingOrganization) {
        // Performance optimization: Avoid redundant query for existingOrganization
        const compiled = compileWorkspaceExperience({
